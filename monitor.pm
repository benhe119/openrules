...
sub getCPU {
  print "getCPU\n";
  my $cmd = `mpstat -I SCPU`;
  return $cmd;
}

sub getSuricataResources {
  print "get Suricata status and resources\n";
  my $cmd = "/etc/init.d/suricata status | grep Main | awk '{system(".'"top -b -n 1 -p "'."\$3)}'";
  print $cmd."\n";
  return `$cmd`;
}
